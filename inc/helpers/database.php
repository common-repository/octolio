...
/**
 * Class inspired by the amazing one called Mysqldump.
 *
 * @category Library
...
class octolio_database_helper
{
    ...
    public function init(
        ...
        $dumpSettingsDefault = [
            ...
            'exclude-tables' => [],
            'include-views' => [],
            'compress' => octolio_database_helper::NONE,
            'init_commands' => [],
            'no-data' => [],
            ...
            'databases' => false,
            'default-character-set' => octolio_database_helper::UTF8,
            'disable-keys' => true,
    ...
    private function connect()
    {
        ...
        try {
            ...
            throw new Exception(
                "Connection to ".$this->dbType." failed with message: ".$e->getMessage()
            );
        }
            ...
            if ($onlyOnce || !$this->dumpSettings['extended-insert']) {
                if ($this->dumpSettings['complete-insert']) {
                    $lineSize += $this->compressManager->write(
                        "INSERT$ignore INTO `$tableName` (".implode(", ", $colNames).") VALUES (".implode(",", $vals).")"
                    );
                } else {
                    $lineSize += $this->compressManager->write(
    ...
    public function prepareListValues($tableName)
    {
        if (!$this->dumpSettings['skip-comments']) {
            $this->compressManager->write(
                "--".PHP_EOL."-- Dumping data for table `$tableName`".PHP_EOL."--".PHP_EOL.PHP_EOL
            );
        }
    ...
    public function endListValues($tableName, $count = 0)
    {
        ...
        if (!$this->dumpSettings['skip-comments']) {
            $this->compressManager->write(
                "-- Dumped table `".$tableName."` with $count row(s)".PHP_EOL.'--'.PHP_EOL.PHP_EOL
            );
        }
...
{
    public static $enums = [
        octolio_database_helper::NONE,
        octolio_database_helper::GZIP,
        octolio_database_helper::BZIP2,
        octolio_database_helper::GZIPSTREAM,
    ];
...
class TypeAdapterMysql extends TypeAdapterFactory
{
    ...
    public function create_view($row)
    {
        ...
        if ($viewStmtReplaced = preg_replace(
            '/^(CREATE(?:\s+ALGORITHM=(?:UNDEFINED|MERGE|TEMPTABLE))?)\s+('.self::DEFINER_RE.'(?:\s+SQL SECURITY DEFINER|INVOKER)?)?\s+(VIEW .+)$/',
    ...
    public function create_procedure($row)
    {
        ...
        if (!isset($row['Create Procedure'])) {
            throw new Exception(
                "Error getting procedure code, unknown output. "."Please check 'https://bugs.mysql.com/bug.php?id=14564'"
            );
        }
    ...
    public function create_function($row)
    {
        ...
        if (!isset($row['Create Function'])) {
            throw new Exception(
                "Error getting function code, unknown output. "."Please check 'https://bugs.mysql.com/bug.php?id=14564'"
            );
        }
    ...
    public function create_event($row)
    {
        ...
        if (!isset($row['Create Event'])) {
            throw new Exception(
                "Error getting event code, unknown output. "."Please check 'http://stackoverflow.com/questions/10853826/mysql-5-5-create-event-gives-syntax-error'"
            );
        }
        $eventName = $row['Event'];
